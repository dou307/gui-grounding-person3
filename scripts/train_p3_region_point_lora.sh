...
python -m src.person3.train_lora_qwen3vl \
  --model "${MODEL_DIR}" \
  --train-file "${PROJECT_ROOT}/data/person3/p3_region_point_train.jsonl" \
  --output-dir "${PROJECT_ROOT}/checkpoints/person3/p3_region_point" \
  --logging-dir "${PROJECT_ROOT}/outputs/logs/person3/p3_region_point" \
  "${EXTRA_ARGS[@]}" \
  "$@"
